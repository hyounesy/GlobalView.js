...
var LABEL_HEIGHT = exports.LABEL_HEIGHT = 12;
var LABEL_WIDTH = exports.LABEL_WIDTH = 16.5;
...
function ImageViewer(gl, globalView) {
  ...
  exports.LABEL_HEIGHT = LABEL_HEIGHT = gl.measureTextHeight() + 2 * LABEL_TEXT_PADDING;
  exports.LABEL_WIDTH = LABEL_WIDTH = gl.measureTextWidth('888') + 2 * LABEL_TEXT_PADDING;
  ...
  this.render = function (flipY, tf) {
    ...
    var refPos = libGlMatrix.vec2.create();
    var imageSize = libGlMatrix.vec2.create();
    ...
    if (options.labelThumbnails) {
      ...
      images.forEach(function (image) {
        ...
        sdrLine.color.apply(sdrLine, _toConsumableArray(image.highlighted ? [1, 1, 0, 1] : image.labelColor ? image.labelColor : defaultImageLabelColor));
        meshLabel.bind(sdrLine, null);
        meshLabel.draw();

        sdrLine.color.apply(sdrLine, _toConsumableArray(image.borderColor ? image.borderColor : defaultImageBorderColor));
        ...
        gl.drawText(_label, refPos[0], refPos[1], 'topright');
        _label += 1;
      });
    } else {
      ...
      images.forEach(function (image) {
        ...
        sdrLine.matWorldViewProj(mattrans);
        sdrLine.color.apply(sdrLine, _toConsumableArray(image.lineColor ? image.lineColor : defaultImageLineColor));
        meshLine.draw();
      });
    }
  ...
  var options = {};
  var defaultImageBorderWidth = 1;
  var defaultImageBorderColor = gl.foreColor;
  var defaultImageLineColor = gl.foreColor;
  var defaultImageLabelColor = gl.backColor;
  ...
  this.resolveIntersections = function (tf) {
    ...
    var b = libGlMatrix.vec2.create();
    var c = libGlMatrix.vec2.create();
    var d = libGlMatrix.vec2.create();
    ...
    for (var _i = 1; _i < images.length; _i += 1) {
      if (images[_i].imagePos) {
        tf.transformPos(a, images[_i].imagePos);
        tf.transformPos(b, images[_i].refPos);
        for (var _j = 0; _j < _i; _j += 1) {
          if (images[_j].imagePos) {
            tf.transformPos(c, images[_j].imagePos);
            tf.transformPos(d, images[_j].refPos);

            if (libAlgorithm.linesIntersect(a, b, c, d)) {
              // console.log("intersection {0} - {1}".format(i, j));
              var _tmp = images[_j].imagePos;
              images[_j].imagePos = images[_i].imagePos;
              images[_i].imagePos = _tmp;
              _i = _j = 0;break; // EDIT: How neccessary is this?
            }
          }
        }
      }
