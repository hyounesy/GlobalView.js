  ...
  this.render = function (flipY, plotBounds) {
    ...
    if (this.visible[1]) {
      ...
      for (var _i = 0; _i < _axis.tickCount; ++_i) {
        ...
        yTickLabel_left = Math.max(yTickLabel_left, gl.measureTextWidth(_tickLabel));
        gl.drawText(_tickLabel, plotBounds.x - _axis.tickLength - 2, gl.height - plotBounds.y - plotBounds.height * _tickPos, 'middleright');
      }
      yTickLabel_left = Math.ceil(plotBounds.x - _axis.tickLength - 10 - yTickLabel_left);
    }
      ...
      case 1:
        {
          ...
          return { 'l': yTickLabel_left - gl.measureTextHeight(), 'r': yTickLabel_left + 2, 't': _plotCenter2 - _halfTextWidth2, 'b': _plotCenter2 + _halfTextWidth2 };
        }
    }
    return null;
  };
