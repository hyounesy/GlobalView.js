...
function CoordinateSystem(gl, globalView) {
  ...
  this.render = function (flipY, plotBounds) {
    ...
    if (this.visible[0]) {
      ...
      libGlMatrix.mat4.translate(mattrans, mattrans, [2 * (plotBounds.x + 0.5) / gl.width - 1, 2 * (plotBounds.y + 0.5) / gl.height - 1, 0]); // 0.5 ... center inside pixel
      libGlMatrix.mat4.rotateZ(mattrans, mattrans, -Math.PI / 2.0);
      libGlMatrix.mat4.scale(mattrans, mattrans, [2 * _axis.tickLength / gl.height, 2 * plotBounds.width / gl.width, 1]);
      sdrLine.matWorldViewProj(mattrans);
      meshLine.draw();
      ...
      meshLine.draw();
      libGlMatrix.mat4.translate(mattrans, mattrans, [0.0, -1.0, 0.0]);
      for (var _i = 0; _i < _axis.tickCount; ++_i) {
        var x = _axis.tickOffset + _i * _axis.tickDistance;
        var _tickPos = (x - _axis.minimum) / (_axis.maximum - _axis.minimum);
        ...
        var _tickLabel = _axis.values ? _axis.values[x] : x.toPrecision(6) / 1;
        gl.drawText(_tickLabel, plotBounds.x + plotBounds.width * _tickPos, gl.height - plotBounds.y + _axis.tickLength + 2, 'topcenter');
      }
      xTickLabel_top = gl.height - plotBounds.y + _axis.tickLength + 10 + gl.measureTextHeight();
    }
  ...
  this.labelFromPoint = function (plotBounds, p) {
    ...
    if (this.visible[1]) {
      ...
      if (p[0] >= yTickLabel_left - gl.measureTextHeight() && p[0] <= yTickLabel_left + 2 && p[1] >= _plotCenter - _halfTextWidth && p[1] < _plotCenter + _halfTextWidth) {
        return 1;
      }
    }
    return null;
