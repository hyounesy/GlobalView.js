  ...
  function DataMesh(gl, posbuffer, numvertices, ndim, options) {
    ...
    this.getPosCode = function (forLineSdr) {
      ...
      var attrDeclCode = '',
          ...
          inputCode = [],
          animatedInputCode = [];
      for (var _d = 0, _i = 0; _d < ndim; _d += 4, ++_i) {
        var attrLen = Math.min(4, ndim - _d);
    ...
    this.drawIndexed = function (texture, idxbuffer, count) {
      for (var _i3 = 0; _i3 < 16; _i3++) {
        ...
        if (gl.ext) {
          gl.ext.vertexAttribDivisorANGLE(_i3, 0);
        }
      }
    ...
    this.drawLines = function (texture, line, offset, count) {
      ...
      if (posbuffer) {
        ...
        for (var d = 0, _i5 = 0; d < ndim; d += 4, ++_i5) {
          if (this.sdrLine.posattr[_i5] !== -1) {
            gl.enableVertexAttribArray(this.sdrLine.posattr[_i5]);
            gl.vertexAttribPointer(this.sdrLine.posattr[_i5], Math.min(4, ndim - d), gl.FLOAT, false, ndim * 4, (offset * ndim + d) * 4);
            gl.ext.vertexAttribDivisorANGLE(this.sdrLine.posattr[_i5], 1);
          }
        }
      }
