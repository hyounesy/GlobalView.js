...
exports.PointViewer = PointViewer;
...
function PointViewer(gl, globalView) {
  var varDataset = void 0;
  var meshDataPoints = null;

  var varPointOpacity = 1.0;
  /*
  ...
  */
  ...
  this.createPointSet = function fCreatePointSet(color, opacity) {
    ...
    if (color) {
      var validationResult = libColormap.validateColormap(color);
  ...
  this.onInputChanged = function fOnInputChanged(activeInputs, animatedInputs, options) {
    activeInputVectors = activeInputs.map(function (i) {
      return varDataset.dataVectors[i];
    });
    animatedInputVectors = animatedInputs.map(function (animatedInput) {
      return varDataset.dataVectors[animatedInput.origin];
    });
    if (meshDataPoints != null) {
      meshDataPoints.recompileShader(options);
    }
  ...
  function DataMesh(glCtx, glbuffer, numvertices, ndim, options) {
    ...
    var linebuffer = glCtx.createBuffer();
    glCtx.bindBuffer(glCtx.ARRAY_BUFFER, linebuffer);
    glCtx.bufferData(glCtx.ARRAY_BUFFER, new Float32Array([0, -1, 0, 1, 2, 1, 2, -1]), glCtx.STATIC_DRAW);
    ...
    var vidbuffer = glCtx.createBuffer();
    ...
    this.getPosCode = function fGetPosCode(forLineSdr) {
      ...
      var attrDeclCode = '';
      var inputs = [/c(\d+)/g, '0.0'];
      var inputCode = [];
      var animatedInputCode = [];
      for (var d = 0, _i = 0; d < ndim; d += 4, _i += 1) {
        var attrLen = Math.min(4, ndim - d);
        attrDeclCode += 'attribute ' + (attrLen === 1 ? 'float' : 'vec' + attrLen) + ' p' + _i + ';\n';
        for (var a = 0; a < attrLen; a += 1) {
          inputs.push('p' + _i + (attrLen === 1 ? '' : '[' + a + ']'));
        }
      }
      var ND = 4; // same as globalView.ND
      for (var _d = 0; _d < ND; _d += 1) {
        inputCode.push(String.prototype.format2.apply(activeInputVectors[_d] ? activeInputVectors[_d].getValueCode : '0.0', inputs));
        animatedInputCode.push(String.prototype.format2.apply(activeInputVectors[_d] ? animatedInputVectors[_d].getValueCode : '0.0', inputs));
      }
    ...
    this.recompileShader = function (vOptions) {
      ...
      // Compile shaders
      this.sdr = new libGraphics.Shader(glCtx, [this.getPosCode(false), libShaders.Shaders.vsDataPoint], ['precision highp float; uniform float pointSize;', opacityMapCoe, libShaders.Shaders.fsDataPoint]);
    ...
    this.draw = function (texture, pOffset, pCount) {
      ...
      for (var _i2 = 0; _i2 < 16; _i2 += 1) {
        glCtx.disableVertexAttribArray(_i2);
        if (glCtx.ext) {
          glCtx.ext.vertexAttribDivisorANGLE(_i2, 0);
        }
      }

      if (posbuffer) {
        glCtx.bindBuffer(glCtx.ARRAY_BUFFER, posbuffer);
        for (var d = 0, _i3 = 0; d < ndim; d += 4, _i3 += 1) {
          if (this.sdr.posattr[_i3] !== -1) {
            glCtx.enableVertexAttribArray(this.sdr.posattr[_i3]);
            glCtx.vertexAttribPointer(this.sdr.posattr[_i3], Math.min(4, ndim - d), glCtx.FLOAT, false, ndim * 4, (offset * ndim + d) * 4);
          }
        }
      }
    ...
    this.drawIndexed = function (texture, idxbuffer, count) {
      for (var _i4 = 0; _i4 < 16; _i4 += 1) {
        glCtx.disableVertexAttribArray(_i4);
        if (glCtx.ext) {
          glCtx.ext.vertexAttribDivisorANGLE(_i4, 0);
        }
      }

      if (posbuffer) {
        glCtx.bindBuffer(glCtx.ARRAY_BUFFER, posbuffer);
        for (var d = 0, _i5 = 0; d < ndim; d += 4, _i5 += 1) {
          if (this.sdr.posattr[_i5] !== -1) {
            glCtx.enableVertexAttribArray(this.sdr.posattr[_i5]);
            glCtx.vertexAttribPointer(this.sdr.posattr[_i5], Math.min(4, ndim - d), glCtx.FLOAT, false, ndim * 4, d * 4);
          }
        }
      }
    ...
    this.drawLines = function (texture, line, pOffset, pCount) {
      ...
      for (var _i6 = 0; _i6 < 16; _i6 += 1) {
        glCtx.disableVertexAttribArray(_i6);
        glCtx.ext.vertexAttribDivisorANGLE(_i6, 0);
      }

      if (posbuffer) {
        glCtx.bindBuffer(glCtx.ARRAY_BUFFER, posbuffer);
        for (var d = 0, _i7 = 0; d < ndim; d += 4, _i7 += 1) {
          if (this.sdrLine.posattr[_i7] !== -1) {
            glCtx.enableVertexAttribArray(this.sdrLine.posattr[_i7]);
            glCtx.vertexAttribPointer(this.sdrLine.posattr[_i7], Math.min(4, ndim - d), glCtx.FLOAT, false, ndim * 4, (offset * ndim + d) * 4);
            glCtx.ext.vertexAttribDivisorANGLE(this.sdrLine.posattr[_i7], 1);
          }
        }
      }
      ...
      libGlMatrix.mat2.scale(lineTransform, lineTransform, libGlMatrix.vec2.fromValues(1 / glCtx.width, 1 / glCtx.height));
      ...
      glCtx.ext.drawArraysInstancedANGLE(glCtx.TRIANGLE_FAN, 0, 4, Math.min(count, numvertices - offset));
    };
