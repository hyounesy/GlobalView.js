  ...
  this.highlightImage = function (image) {
    ...
    if (libUtility.isNumber(image)) {
      for (var _i = 0; _i < images.length; ++_i) {
        images[_i].highlighted = _i === image;
      }
    } else {
      for (var _i2 = 0; _i2 < images.length; ++_i2) {
        images[_i2].highlighted = images[_i2] === image;
      }
    }
    this.invalidate();
  };
  ...
  libUtility.addMouseUpHandler(function (event) {
    ...
    if (mousePolygon !== null) {
      if (this['onSelectionChanged'] !== null && mousePolygon.length >= 3) {
        ...
        for (var _i3 = 0; _i3 < mousePolygon.length; ++_i3) {
          var _p = mousePolygon[_i3];

          // Transform p from canvas space to device coordinates
          _p[0] = 2 * _p[0] / canvas.width - 1;
          _p[1] = 1 - 2 * _p[1] / canvas.height;
        ...
        pointViewer.points.forEach(function (i) {
          var px = _v.getValue(i);
          var py = _v2.getValue(i);
          if (libAlgorithm.pointInsidePolygon([px, py], mousePolygon)) {
            _selection.push(i);
          }
        });
        this['onLassoSelection'](dataset, _selection, mousePolygon);
      }
