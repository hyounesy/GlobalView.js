...
var libPointViewer = require('./pointViewer.js');
var libImageViewer = require('./imageViewer');
var libDensityViewer = require('./densityViewer.js');
var libHistogramViewer = require('./histogramViewer.js');
var libCoordinateSystem = require('./coordinateSystem.js');
var libColormap = require('./colormap.js');
 ...
 * }} */
var OptionDescription = void 0; // eslint-disable-line no-unused-vars
...
function GlobalView(div, startupOptions) {
  ...
  var canvas = null;
  for (var i = 0; i < div.children.length; i += 1) {
  ...
  this.updateColorSchema = function () {
    var vDivStyle = window.getComputedStyle(div);
    gl.backColor = vDivStyle.backgroundColor === 'transparent' ? [0, 0, 0, 0] : libUtility.rgbStringToFloatArray(vDivStyle.backgroundColor);
    gl.foreColor = libUtility.rgbStringToFloatArray(gl.foreColorString = vDivStyle.color);
    gl.clearColor.apply(gl, _toConsumableArray(gl.backColor));
  ...
  function Transform() {
    ...
    var scales = new Float32Array(ND);
    var animatedScales = new Float32Array(ND);
    var invalid = false;
    ...
    this.deviceCoordToDatasetCoord = function (vOutCoord, vInCoord) {
      ...
      for (var d = 0, nd = Math.min(vIn.length, vOut.length, ND); d < nd; d += 1) {
    ...
    this.transformPos = function (vOutCoord, vInCoord) {
      ...
      for (var d = 0, nd = vOut.length; d < nd; d += 1) {
  ...
  function setPlotBounds(padding) {
    var computedPadding = void 0;
    if (libUtility.isArray(padding) && padding.length === 4) {
      computedPadding = padding.map(function (v, i) {
      ...
      computedPadding = Array.create(4, function (i) {
    ...
    if (newPlotBounds.x !== plotBounds.x || newPlotBounds.y !== plotBounds.y || newPlotBounds.width !== plotBounds.width || newPlotBounds.height !== plotBounds.height) {
  ...
  */
    ...
    if (dataset) {
      dataset.iterateDensityMaps(function (densityMap) {
        if (densityMap.stencilMap && densityMap.stencilMap.data) {
          for (var _i = 0, stencilMap = densityMap.stencilMap.data, len = stencilMap.length; _i < len; _i += 1) {
            stencilMap[_i] = 0;
          }
        }
      });
      ...
      if (dataset.imageFilenames) {
        pointViewer.representativePoints.forEach(function (r) {
          if (dataset.imageFilenames[r]) {
            ...
            var p = libAlgorithm.findClosePointOfLowDensity(dataset, d0, d1, r, densityMap, densityMap.stencilMap, 0.6 * options.thumbnailSize / gl.width, 0.6 * (options.thumbnailSize + libImageViewer.getLabelHeight()) / gl.height); // EDIT: Factor 0.6: WHY?
            imagePos[d0] = p[0];
            imagePos[d1] = p[1];
  ...
  this.showImage_lowDensity = function (index) {
    if (dataset.imageFilenames && dataset.imageFilenames[index]) {
      ...
      dataset.requestDensityMap(d0, d1, undefined, undefined, function (pDensityMap) {
        ...
        var imageHeight = (0.6 * options.thumbnailSize + libImageViewer.getLabelHeight()) / gl.height; // EDIT: Factor 0.6: WHY?
        if (d1 < d0) {
          // Swap d0 <-> d1
          var temp = d0;
          d0 = d1;
        ...
        var imagePos = void 0;
        if (libUtility.isUndefined(densityMap.data)) {
          // If densityMap is nD
          imagePos = libAlgorithm.findClosePointOfLowDensityNDDescend(dataset, index, densityMap, 0.6 * options.thumbnailSize / Math.min(gl.width, gl.height));
        } else {
          // EDIT: Factor 0.6: WHY?
  ...
  this.showImages_project = function (points) {
    ...
    var eigenVecLength = Math.sqrt(eigenvec[0] * eigenvec[0] + eigenvec[1] * eigenvec[1]);
    ...
    var labelHeightOffset = 1.0 + libImageViewer.getLabelHeight() / options.thumbnailSize;
    var labelWidthOffset = 1.0 + (libImageViewer.getLabelHeight() + 2 * libImageViewer.getLabelWidth()) / options.thumbnailSize;
    ...
    var posToLoc = function posToLoc(pos) {
      var p = pos;
    ...
    var locToPos = function locToPos(loc) {
      var l = (loc + 4) % 4;
      var p = void 0;
      var li = Math.floor(l);
      switch (li) {
    ...
    var v1 = dataset.dataVectors[activeInputs[1]];
    points.forEach(function (p) {
    ...
    var detectOverlap = function detectOverlap(R, overlapThreshold) {
      var P = [];
      for (var j = 1; j < R.length; j += 1) {
        for (var _i2 = 0; _i2 < j; _i2 += 1) {
          if (Math.abs(R[_i2] - R[j]) < overlapThreshold) {
            P.push([_i2, j]);
          }
        }
      }
    ...
    var removeOverlap = function removeOverlap(pR, i, j, rank, overlapThreshold) {
      var R = pR;
    ...
    if (maxNumIterations !== 0) {
      (function () {
        ...
        for (var iter = 0; iter < maxNumIterations && P.length !== 0; iter += 1) {
          // TODO: Shuffle P
        ...
        var repairedR = new Array(R.length);
        for (var _i3 = 0; _i3 < R.length; _i3 += 1) {
          repairedR[rank[_i3]] = R[newRank[_i3]];
        }
  ...
  this.highlightImage = function (image) {
    ...
    if (libUtility.isNumber(image)) {
      for (var _i4 = 0; _i4 < images.length; _i4 += 1) {
        images[_i4].highlighted = _i4 === image;
      }
    } else {
      for (var _i5 = 0; _i5 < images.length; _i5 += 1) {
        images[_i5].highlighted = images[_i5] === image;
      }
    }
    this.invalidate();
  };
  ...
  var pointDragDownPos = null;
  var viewDragStartPos = null;
  var viewDragX = void 0;
  var viewDragY = void 0;
  var viewDragZ = void 0;
  var mouseOverAxisLabel = null;
  var mouseOverImage = null;
  var imageDragStartPos = null;
  var imageDragImages = [];
  ...
  canvas.onmousedown = function (event) {
    ...
    var sqscl1 = tf.getScale(1) * tf.getScale(1);
    var v0 = dataset.dataVectors[activeInputs[0]];
    var v1 = dataset.dataVectors[activeInputs[1]];
  ...
  libUtility.addMouseMoveHandler(onmousemove = function (event) {
    if (tf === null || offscreenRendering !== null || event.target !== canvas && pointDragDownPos === null && viewDragStartPos === null && imageDragStartPos === null && mouseRect === null && mousePolygon === null) {
      return;
    }
  ...
  libUtility.addMouseUpHandler(function (event) {
    ...
    if (mousePolygon !== null) {
      if (_this.onSelectionChanged !== null && mousePolygon.length >= 3) {
        // TODO: Find points within mousePolygon -> selection

        // Transform mousePolygon from canvas space to dataset coordinates
        for (var _i6 = 0; _i6 < mousePolygon.length; _i6 += 1) {
          var p = mousePolygon[_i6];
        ...
        var v1 = dataset.dataVectors[activeInputs[1]];
        pointViewer.points.forEach(function (i) {
          var px = v0.getValue(i);
          var py = v1.getValue(i);
          if (libAlgorithm.pointInsidePolygon([px, py], mousePolygon)) {
            selection.push(i);
          }
        });
        _this.onLassoSelection(dataset, selection, mousePolygon);
      }
    ...
    if (mouseRect !== null) {
      if (_this.onSelectionChanged !== null && mouseRect.width !== 0 && mouseRect.height !== 0) {
        ...
        var _p = new Float32Array([mouseRect.l, mouseRect.t]);
        tf.deviceCoordToDatasetCoord(_p, _p);
        mouseRect.l = _p[0];mouseRect.t = _p[1];
        _p = new Float32Array([mouseRect.r, mouseRect.b]);
        tf.deviceCoordToDatasetCoord(_p, _p);
        mouseRect.r = _p[0];mouseRect.b = _p[1];

        var px = void 0;
        var py = void 0;
        var _selection = [];
        var _v = dataset.dataVectors[activeInputs[0]];
        var _v2 = dataset.dataVectors[activeInputs[1]];
        pointViewer.points.forEach(function (i) {
          px = _v.getValue(i);
          py = _v2.getValue(i);
        ...
        _this.onLassoSelection(dataset, _selection, mouseRect);
      }
  ...
  this.enableOffscreenRendering = function (width, height) {
    ...
    // Bind framebuffer
    gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, offscreenRendering.rttTexture, 0);
    // Bind depth buffer
