...
function onLoad() {
	...
	var DATASETS = [{ name: "10 random points", create: function create() {
			return new RandomDataset(10, 3, dataset_onLoad);
		} }, { name: "100 random points", create: function create() {
			return new RandomDataset(100, 3, dataset_onLoad);
		} }, { name: "1.000 random points", create: function create() {
			return new RandomDataset(1000, 3, dataset_onLoad);
		} }, { name: "10.000 random points", create: function create() {
			return new RandomDataset(10000, 3, dataset_onLoad);
		} }, { name: "100.000 random points", create: function create() {
			return new RandomDataset(100000, 3, dataset_onLoad);
		} }, { name: "1.000.000 random points", create: function create() {
			return new RandomDataset(1000000, 3, dataset_onLoad);
		} }, { name: "10.000.000 random points", create: function create() {
			return new RandomDataset(10000000, 3, dataset_onLoad);
		} }, { name: "iris", url: "datasets/iris.data", create: function create() {
			return new CsvDataset("datasets/iris.data", { columnLabels: ["Sepal Length [cm]", "Sepal Width [cm]", "Petal Length [cm]", "Petal Width [cm]", "Class"] }, dataset_onLoad);
		} }, { name: "allencell", url: "datasets/AICS_Cell-feature-analysis_v1.5.csv", create: function create() {
			return new CsvDataset("datasets/AICS_Cell-feature-analysis_v1.5.csv", { hasHeader: true, nameColumn: 1, imageFilenames: function imageFilenames(data) {
					return "datasets/AICS_Cell-feature-analysis_v1.5_images/" + data[1] + ".png";
				} }, dataset_onLoad);
		} }, { name: "allencell x2", url: "datasets/AICS_Cell-feature-analysis_v1.5_x2.csv", create: function create() {
			return new CsvDataset("datasets/AICS_Cell-feature-analysis_v1.5_x2.csv", { hasHeader: true, nameColumn: 1, imageFilenames: function imageFilenames(data) {
					return "datasets/AICS_Cell-feature-analysis_v1.5_images/" + data[1] + ".png";
				} }, dataset_onLoad);
		} }, { name: "allencell x10", url: "datasets/AICS_Cell-feature-analysis_v1.5_x10.csv", create: function create() {
			return new CsvDataset("datasets/AICS_Cell-feature-analysis_v1.5_x10.csv", { hasHeader: true, nameColumn: 1, imageFilenames: function imageFilenames(data) {
					return "datasets/AICS_Cell-feature-analysis_v1.5_images/" + data[1] + ".png";
				} }, dataset_onLoad);
		} }, { name: "allencell x100", url: "datasets/AICS_Cell-feature-analysis_v1.5_x100.csv", create: function create() {
			return new CsvDataset("datasets/AICS_Cell-feature-analysis_v1.5_x100.csv", { hasHeader: true, nameColumn: 1, imageFilenames: function imageFilenames(data) {
					return "datasets/AICS_Cell-feature-analysis_v1.5_images/" + data[1] + ".png";
				} }, dataset_onLoad);
		} }];
	...
	globalView.onMouseDown = function (event) {
		switch (event.button) {
			...
			case 0:
				...
				if (ctrlPressed) event['viewDragging'] = true;else event['polygonLassoSelection'] = true;
				break;
	...
	DATASETS.forEach(function (dataset) {
		if (isUndefined(dataset.url)) {
			...
			if (cbDataset.options.length === DATASETS.length) {
				...
				if (datasetIndex !== null) cbDataset.selectedIndex = datasetIndex;
...
function cbDataset_onChange() {
	...
	if (cbDataset.selectedIndex >= 0 && cbDataset.options[cbDataset.selectedIndex].createDataset) cbDataset.options[cbDataset.selectedIndex].createDataset();
...
function dataset_onLoad(_dataset) {
	...
	var activeColumnX = readIntCookie('activeColumnX'),
	    activeColumnY = readIntCookie('activeColumnY'),
	    activeColumnC = readIntCookie('activeColumnC'),
	    activeColumnS = readIntCookie('activeColumnS');
...
function cbShowDensity_onChange(sender) {
	if (sender.checked) requestVariance(densityMapOptions.gaussScale, true);
...
function requestVariance(variance, fast) {
	...
	dataset.requestDensityMap(globalView.getActiveColumn(0), globalView.getActiveColumn(1), fast ? 64 : null, densityMapOptions, function (densityMap) {
		if (densityMapOptions.gaussScale === variance) {
			...
			if (fast) requestVariance(variance, densityMap.options.gaussScale !== variance);
			/*else
   {
...
function onMouseOverDatapoint(dataset, index) {
	if (index === -1) {
		...
		if (dataset.data) {
			...
			pDataPoint.innerText = dataset.dataVectors.map(function (dataVector) {
				return dataVector.label + ": " + (dataVector.values ? dataVector.values[Math.floor(dataVector.getValue(index))] : dataVector.getValue(index));
			}).join("\n");
		} else pDataPoint.innerText = dataset.names ? dataset.names[index] : "datapoint " + index;
	}
}
function onSelectionChanged(dataset, selection) {
	if (selection.length === 0) {
		if (!shiftPressed) globalView.selectedPoints.clear();
	} else {
		if (shiftPressed) globalView.selectedPoints.append(selection);else if (selection.length !== 1 || !globalView.selectedPoints.contains(selection[0])) globalView.selectedPoints.assign(selection);
	}
}
