...
function DensityViewer(gl, globalView) {
  ...
  this.render = function (flipY, tf, d0, d1) {
    ...
    if (this.showClusterMap) {
      if (dataset && dataset.isClusterMapReady(d0, d1)) {
        ...
        if (!texture) {
          var _densityMap = this.showDensityMap ? dataset.requestDensityMap(d0, d1, undefined, undefined) : null; // Retrieve densityMap synchronously (since we already know it's ready)
          ...
          for (var i = 0; i < clusterMap.data.length; ++i) {
            ...
            if (c === 0) {
              ...
              --c; // --c ... ID to index
              var d = _densityMap ? (_densityMap.data[i] - clusterMap.minDensities[c]) / (clusterMap.densities[c] - clusterMap.minDensities[c]) : 0.75;
              if (d < 0.0) {
                d = 0.0;
              }
        ...
        sdrClusterMap.matWorldViewProj(_mattrans);
  ...
  this.updateImages = function (images, d0, d1) {
    ...
    for (var _i = 0; _i < bodies.length; ++_i) {
      var sample_x = Math.floor(bodies[_i].x),
          sample_y = Math.floor(bodies[_i].y);
      ...
      if (bestDir !== null) {
        repellPoint(bodies[_i], bodies[_i].x + bestDir[0], bodies[_i].y + bestDir[1], Number.MIN_VALUE, 0, 0.0, density);
        console.log(density);
      }
    }
