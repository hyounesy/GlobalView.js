...
function computeHistogram2D(dataset, d0, d1, width, height) {
  ...
  for (var i = 0; i < n; i += 1) {
    ...
    var hvalue = histogram[Math.min(height - 1, p1) * width + Math.min(width - 1, p0)] += 1;
    if (hvalue > maximum) {
      maximum += 1;
    } // maximum can only grow by 1, so we know histogram[...] == maximum + 1
  }
...
function sampleDensityMapChain(densityMapChain) {
  ...
  do {
    ...
    for (i = sampleM + 1; i < chainLength && !isNaN(lastSample); i += 1) {
      sample[i + 1] = lastSample = sampleDensityMapColumn(densityMapChain[i], lastSample, i - sampleM);
    }
