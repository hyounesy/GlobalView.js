...
function computeClusterMap_method3(densityMap, d0, d1, options) {
  ...
  if (false) {
    ...
    for (var y = 0; y < height; ++y) {
      for (var x = 0; x < width; ++x) {
        if (clustermap[y * width + x] !== 0 && (x < width - 1 && clustermap[y * width + x - 1] === 0 || x > 0 && clustermap[y * width + x + 1] === 0 || y > 0 && clustermap[(y - 1) * width + x] === 0 || y < height - 1 && clustermap[(y + 1) * width + x] === 0)) {
