...
exports.findClosePointOfLowDensityDescend = findClosePointOfLowDensityDescend;
exports.findClosePointOfLowDensityNDDescend = findClosePointOfLowDensityNDDescend;
...
var libPathFinding = require('./pathFinding.js');
...
function DensityMapOptions(source) {
  if (source) {
    ...
    // this.targetWidth = this.targetHeight = 1024;
    /**
...
function computeHistogram2D(dataset, d0, d1, width, height) {
  ...
  for (var i = 0; i < n; i += 1) {
    ...
    var b = Math.min(height - 1, p1) * width + Math.min(width - 1, p0);
    histogram[b] += 1;
...
function computeDensityMap(histogram, options) {
  ...
  var maxDensity = histogram.maximum;
  ...
  var newBoundsL = Number.MAX_VALUE;
  var newBoundsR = Number.MIN_VALUE;
  var newBoundsT = Number.MAX_VALUE;
  var newBoundsB = Number.MIN_VALUE;
  tik();
  if (inflateToFit) {
  ...
  while (expectedRuntime > options.maxExpectedRuntime && width >= 2 && height >= 2) {
    ...
    var downScaledWidth = Math.floor(width / 2);
    var downScaledHeight = Math.floor(height / 2);
    // libUtility.consoleLog(
      ...
      for (var _y3 = 0; _y3 < height; _y3 += 1) {
        for (var _x3 = 0; _x3 < width; _x3 += 1) {
          if (initialDensities[_y3 * width + _x3] !== 0.0) {
            ...
            newBoundsL = Math.min(newBoundsL, _x3 - _r2);
            newBoundsR = Math.max(newBoundsR, _x3 + _r2);
            newBoundsT = Math.min(newBoundsT, _y3 - _r2);
            newBoundsB = Math.max(newBoundsB, _y3 + _r2);
          }
        }
      }
    } else {
  ...
  /*
  // Precompute extends of gaussians up to maxDensity based on
  // cutoffIntensity and normalizedGaussScale -> ext[]
  ...
  for (var i = 0; i < maxDensity; i+=1) {
    ...
    // Set upper bound for ext[i]:
    // Precomputed map of gaussian scales shouldn't be larger than densityMap (size*size)
    ext[i] = Math.min(ext[i], maxExtend);
  } */

  // Compute extend of the largest gaussian based on cutoffIntensity and
  // normalizedGaussScale -> maxExtend
  ...
  maxExtend = Math.min(Math.floor(Math.sqrt(Math.log(cutoffIntensity / maxDensity) / normalizedGaussScale)), maxExtend);
    ...
    for (var _y6 = 0; _y6 < height; _y6 += 1) {
      for (var _x6 = 0; _x6 < width; _x6 += 1) {
        if (initialDensities[_y6 * width + _x6] !== 0.0) {
          var initialDensitiesXY = initialDensities[_y6 * width + _x6];

          // Compute extend of gaussian with value initialDensities_xy -> yExtend
          var yExtend = Math.sqrt(Math.log(cutoffIntensity / initialDensitiesXY) / normalizedGaussScale);
          yExtend = Math.min(Math.floor(yExtend), maxExtend) - 1;
          if (yExtend <= 0) {
            densities[_y6 * densitMapWidth + _x6] += initialDensitiesXY;
          ...
          for (var yy = _y6 - yExtend, yend = _y6 + yExtend; yy <= yend; yy += 1) {
            ...
            for (var xx = _x6 - xExtend, xend = _x6 + xExtend; xx <= xend; xx += 1) {
              densities[(yy - newBoundsT) * densitMapWidth + xx - newBoundsL] += initialDensitiesXY * gauss[Math.abs(_y6 - yy) * maxExtend + Math.abs(_x6 - xx)];
            }
          }
        }
      }
    ...
    for (var _y7 = 0; _y7 < height; _y7 += 1) {
      for (var _x7 = 0; _x7 < width; _x7 += 1) {
        if (initialDensities[_y7 * width + _x7] !== 0.0) {
          var _initialDensitiesXY = initialDensities[_y7 * width + _x7];

          // Compute extend of gaussian with value initialDensities_xy -> yExtend
          var _yExtend = Math.sqrt(Math.log(cutoffIntensity / _initialDensitiesXY) / normalizedGaussScale);
          _yExtend = Math.min(Math.floor(_yExtend), maxExtend) - 1;
          if (_yExtend <= 0) {
            densities[_y7 * densitMapWidth + _x7] += _initialDensitiesXY;
            continue; // eslint-disable-line no-continue
          }
          // var yExtend = ext[initialDensities_xy - 1] - 1; // Get precomputed yExtend
          ...
          for (var _yy = Math.max(0, _y7 - _yExtend), _yend = Math.min(densitMapHeight - 1, _y7 + _yExtend); _yy <= _yend; _yy += 1) {
            ...
            for (var _xx = Math.max(0, _x7 - _xExtend), _xend = Math.min(densitMapWidth - 1, _x7 + _xExtend); _xx <= _xend; _xx += 1) {
              densities[_yy * densitMapWidth + _xx] += _initialDensitiesXY * gauss[Math.abs(_y7 - _yy) * maxExtend + Math.abs(_x7 - _xx)];
            }
          }
        }
      }
  ...
  for (var _y8 = 0, i = 0; _y8 < densitMapHeight; _y8 += 1) {
    for (var _x8 = 0; _x8 < densitMapWidth; _x8 += 1, i += 1) {
      ...
      if (density > 0.0) {
        ...
        newBoundsL = Math.min(newBoundsL, _x8);
        newBoundsR = Math.max(newBoundsR, _x8);
        newBoundsT = Math.min(newBoundsT, _y8);
        newBoundsB = Math.max(newBoundsB, _y8);
      } else {
        densities[i] = 0.0;
      }
    }
  ...
  if (options.shrinkToFit) {
    ...
    var resizedDensitMapWidth = Math.max(0, newBoundsR - newBoundsL + 1);
    var resizedDensitMapHeight = Math.max(0, newBoundsB - newBoundsT + 1);
    ...
    if (resizedDensities.length !== 0) {
      for (var _y9 = 0, _i = 0, j = newBoundsL + newBoundsT * densitMapWidth; _y9 < resizedDensitMapHeight; _y9 += 1, j += densitMapWidth - resizedDensitMapWidth) {
 ...
 * @package
 * @param  {Dataset} dataset
 * @param  {number} d0 The first input dimension
 *                     (This value must match the one used to compute the histogram)
 * @param  {number} d1 The second input dimension
 *                     (This value must match the one used to compute the histogram)
...
function findRepresentativePoints(dataset, d0, d1, densityMap, k, dist, targetRatio) {
  ...
  var dHigh = indices.length - 1;
  var dLow = 0;
  var pointIsHigh = void 0;
  var numHighRepresentativePoints = 0;
  var ratio = 0.5; // Initial ratio is "fifty-fifty"
  ...
  var _loop = function _loop() {
    var di = indices[next()];
    var di0 = densityMap.transformX(v0.getValue(di)) / densityMap.width;
    var di1 = densityMap.transformY(v1.getValue(di)) / densityMap.height;
  ...
  while (dHigh >= dLow && representativePoints.length < k) {
    _loop();
  }
...
function findRepresentativePoints2(dataset, d0, d1, densityMap, k, targetRatio) {
  var varK = k;
  var varTargetRatio = targetRatio;
 ...
 * @return {Array<number>} An array of up to k point indices of characteristic points
 */
function findRepresentativePointsND(dataset, densityMap, numPointsToReturn, dist) {
  ...
  var representativePoints = [indices[dHigh]];
  dHigh -= 1;
 ...
 * @return {Array<number>} An array of up to k point indices of characteristic points
 */
function findRepresentativePointsND2(dataset, densityMap, numPointsToReturn) {
  var k = Math.min(numPointsToReturn, dataset.length);
  var dist = 0.2;
  var representativePoints = void 0;
  // eslint-disable-next-line no-cond-assign
...
function findClosePointOfLowDensity(dataset, d0, d1, p, densityMap, stencilMap, minDistX, minDistY) {
  ...
  var minDMDistX = Math.ceil(minDistX * width);
  var minDMDistY = Math.ceil(minDistY * height);
  ...
  // TODO: -1 ... Why?
  var xmin = Math.min(0, Math.floor(densityMap.transformX(v0.minimum)) - minDMDistX - 1);
  // TODO: +2 ... Why?
  var xmax = Math.max(width, Math.ceil(densityMap.transformX(v0.maximum)) + minDMDistX + 2);
  // TODO: -1 ... Why?
  var ymin = Math.min(0, Math.floor(densityMap.transformY(v1.minimum)) - minDMDistY - 1);
  // TODO: +2 ... Why?
  var ymax = Math.max(height, Math.ceil(densityMap.transformY(v1.maximum)) + minDMDistY + 2);
  var stencilStride = xmax - xmin;

  // Create stencilMap if it doesn't exist
  var varStencilMap = stencilMap;
  if (!varStencilMap.data) {
    varStencilMap.data = new Uint8Array((varStencilMap.width = xmax - xmin) * (varStencilMap.height = ymax - ymin));
  }
  var stencil = varStencilMap.data;

  // Mark p in stencil map
  var imgxmin = Math.max(xmin, Math.floor(p0) - minDMDistX);
  var imgxmax = Math.min(xmax, Math.floor(p0) + minDMDistX);
  var imgymin = Math.max(ymin, Math.floor(p1) - minDMDistY);
  var imgymax = Math.min(ymax, Math.floor(p1) + minDMDistY);
  ...
  var sqMinDistX = minDMDistX * minDMDistX;
  var sqMinDistY = minDMDistY * minDMDistY;
  var sqDensityOffset = densityOffset * densityOffset;

  var closestPoint = null;
  var closestPointPenalty = Number.MAX_VALUE;
  var sqdx = void 0;
  var sqdy = void 0;
...
function markPointsInStencilMap(dataset, d0, d1, points, densityMap, stencilMap, minDistX, minDistY) {
  ...
  points.forEach(function (p) {
    ...
    var imgxmin = Math.max(xmin, p0 - minDMDistX);
    var imgxmax = Math.min(xmax, p0 + minDMDistX);
    var imgymin = Math.max(ymin, p1 - minDMDistY);
    var imgymax = Math.min(ymax, p1 + minDMDistY);
    for (var y = imgymin; y < imgymax; y += 1) {
      for (var x = imgxmin; x < imgxmax; x += 1) {
        stencil[(y - ymin) * stencilStride + (x - xmin)] = 1;
      }
 ...
 */
function downloadStencilMap(stencilMap, outputFileName) {
 ...
 */
function findClosePointOfLowDensityDescend(dataset, d0, d1, p, densityMap, minDistX, minDistY) {
  var data = dataset.fdata;
  ...
  var xMin = minDMDistX;
  var xMax = width - minDMDistX;
  var yMin = minDMDistY;
  var yMax = height - minDMDistY;
 ...
 */
function findClosePointOfLowDensityNDDescend(dataset, p, densityMap, minDist) {
  var data = dataset.fdata;
  // const n = dataset.length;
  var nc = dataset.numColumns;
  var size = densityMap[0][0].width;
  ...
  densityOffset *= size;
  densityScale *= size;
  var minDistSize = Math.ceil(minDist * size);

  // Define overall bounds
  var min = minDistSize;
  var max = size - minDistSize;
  ...
  var computePenalty = function computePenalty(pty) {
    var sqDensity = 0.0;
    for (var d0 = 0; d0 < nc; d0 += 1) {
      for (var d1 = d0 + 1; d1 < nc; d1 += 1) {
        sqDensity += Math.pow(densityOffset + densityMap[d0][d1 - d0 - 1].data[pty[d1] * size + pty[d0]] * densityScale, 2);
      }
    }
    var sqDist = pty.reduce(function (ax, pt, pi) {
      var dp = Math.abs(pt - start[pi]);
      return ax + (dp > minDistSize ? Math.pow(dp - minDistSize, 2) : Math.pow(minDistSize - dp, 2));
    });
    return sqDensity + sqDist;
  };

  var bestState = { penalty: Number.MAX_VALUE };
  var maxIterations = 100; // 5000;
  var searchProblem = {
    getStartState: function getStartState() {
      var startArray = new Float32Array(nc);
      for (var _c5 = 0; _c5 < nc; _c5 += 1) {
        startArray[_c5] = Math.max(min, Math.min(max - 1, Math.floor(start[_c5])));
      }
      return { p: startArray };
    },
    isGoalState: function isGoalState() /* state */{
    ...
    forEachSuccessor: function forEachSuccessor(state, onSuccessor) {
      actions.forEach(function (action) {
        var pa = new Float32Array(nc);
        for (var _c6 = 0; _c6 < nc; _c6 += 1) {
          pa[_c6] = state.p[_c6] + action[_c6];
          if (pa[_c6] < min || pa[_c6] >= max) {
            return;
          }
        }
        var newState = { pa: pa, penalty: computePenalty(pa) };
  ...
  /*
  var xMin = Math.max(0, closestPoint[0] - 2 * minDistX),
  xMax = Math.min(size, closestPoint[0] + 2 * minDistX);
  var yMin = Math.max(0, closestPoint[1] - 2 * minDistY),
  yMax = Math.min(size, closestPoint[1] + 2 * minDistY);
 ...
 */
function sampleDensityMapRow(densityMap, sampleCol, maxIterations) {
  var varMaxIterations = maxIterations;
  ...
  var height = densityMap.height;
  var scale = densityMap.maximum;
  var sampleY = Math.floor(sampleCol) * height;
 ...
 */
function sampleDensityMapColumn(densityMap, sampleRow, maxIterations) {
  var varMaxIterations = maxIterations;
  if (libUtility.isUndefined(varMaxIterations)) {
    varMaxIterations = Number.MAX_SAFE_INTEGER;
  }
  ...
  var height = densityMap.height;
  var scale = densityMap.maximum;
  var sampleX = Math.floor(sampleRow);
  var sampleY = void 0;
  var sampleD = void 0;
  ...
  do {
    ...
    sampleD = Math.random() * scale;
    varMaxIterations -= 1;
  } while (varMaxIterations && densityMap.data[Math.floor(sampleY) * width + sampleX] < sampleD);
 ...
 *        `[DensityMap(d0=0, d1=1), DensityMap(d0=1, d1=2) ... DensityMap(d0=n-2, d1=n-1)]`
...
function sampleDensityMapChain(densityMapChain) {
  ...
  var sampleM = Math.floor(Math.random() * chainLength);

  var i = void 0;
  var lastSample = void 0;
  do {
    // Sample the initial map
    var initialSamples = sampleDensityMap(densityMapChain[sampleM]);
...
function computeClusterMap(densityMap, d0, d1, options) {
  ...
  var enableExtensionToZeroDensity = false;
  if (enableExtensionToZeroDensity) {
    ...
    var neighborQueue = new PriorityQueue('d');
    ...
    for (var _y12 = 0; _y12 < height; _y12 += 1) {
      for (var _x12 = 0; _x12 < width; _x12 += 1) {
        if (clustermap[_y12 * width + _x12] !== 0 && (_x12 < width - 1 && clustermap[_y12 * width + _x12 - 1] === 0 || _x12 > 0 && clustermap[_y12 * width + _x12 + 1] === 0 || _y12 > 0 && clustermap[(_y12 - 1) * width + _x12] === 0 || _y12 < height - 1 && clustermap[(_y12 + 1) * width + _x12] === 0)) {
          neighborQueue.push({
            c: clustermap[_y12 * width + _x12], x: _x12, y: _y12, d: densities[_y12 * width + _x12]
          });
        }
      }
    }
    ...
    while (neighborQueue.length) {
      ...
      if (_x13 !== -1) {
        ...
        if (nd !== 0 && clustermap[_y13 * width + _x13] === 0) {
          neighborQueue.push({
            c: clustermap[_y13 * width + _x13] = id, x: _x13, y: _y13, d: nd = densities[_y13 * width + _x13]
          });
      ...
      if (_x13 !== width) {
        ...
        if (_nd !== 0 && clustermap[_y13 * width + _x13] === 0) {
          neighborQueue.push({
            c: clustermap[_y13 * width + _x13] = id, x: _x13, y: _y13, d: _nd = densities[_y13 * width + _x13]
          });
      ...
      if (_y13 !== -1) {
        ...
        if (_nd2 !== 0 && clustermap[_y13 * width + _x13] === 0) {
          neighborQueue.push({
            c: clustermap[_y13 * width + _x13] = id, x: _x13, y: _y13, d: _nd2 = densities[_y13 * width + _x13]
          });
      ...
      if (_y13 !== height) {
        ...
        if (_nd3 !== 0 && clustermap[_y13 * width + _x13] === 0) {
          neighborQueue.push({
            c: clustermap[_y13 * width + _x13] = id, x: _x13, y: _y13, d: _nd3 = densities[_y13 * width + _x13]
          });
 ...
 */
function downloadDensityMap(densityMap, outputFileName) {
  var fileName = outputFileName;
  if (!fileName) {
