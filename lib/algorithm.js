...
function computeDensityMap(histogram, options) {
  ...
  var newBounds_r = Number.MIN_VALUE;
  var newBounds_t = Number.MAX_VALUE;
  var newBounds_b = Number.MIN_VALUE;
  tik();
  if (inflateToFit) {
  ...
  newBounds_l = densitMapWidth - 1;
  newBounds_r = 0;
  newBounds_t = densitMapHeight - 1;
  newBounds_b = 0;
...
function findRepresentativePointsND(dataset, densityMap, numPointsToReturn, dist) {
  ...
  var representativePoints = [indices[d_high]];
  d_high -= 1;
