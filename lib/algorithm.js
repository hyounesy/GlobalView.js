...
function computeDensityMap(histogram, options) {
  ...
  if (inflateToFit) {
    for (var _y = 0; _y < height; ++_y) {
      for (var _x = 0; _x < width; ++_x) {
        if (initialDensities[_y * width + _x] !== 0.0) {
          ...
          expectedRuntime += _r * _r * Math.PI;
          newBounds_l = Math.min(newBounds_l, _x - _r);
          newBounds_r = Math.max(newBounds_r, _x + _r);
          newBounds_t = Math.min(newBounds_t, _y - _r);
          newBounds_b = Math.max(newBounds_b, _y + _r);
        }
      }
    }
  } else {
    for (var _y2 = 0; _y2 < height; ++_y2) {
      for (var _x2 = 0; _x2 < width; ++_x2) {
        if (initialDensities[_y2 * width + _x2] !== 0.0) {
          var _r2 = Math.floor(Math.sqrt(Math.log(cutoffIntensity / initialDensities[_y2 * width + _x2]) / normalizedGaussScale)) - 1;
          _r2 = Math.max(1, Math.min(maxExtend, _r2));

          var _trimX = Math.min(_r2, Math.min(_x2, width - _x2)) / _r2;
          var _trimY = Math.min(_r2, Math.min(_y2, height - _y2)) / _r2;
          _trimX = 0.5 + 0.5 * _trimX * _trimX;
          _trimY = 0.5 + 0.5 * _trimY * _trimY;
          expectedRuntime += _r2 * _r2 * Math.PI * _trimX * _trimY;
        }
      }
    }
  }
  ...
  while (expectedRuntime > options['maxExpectedRuntime'] && width >= 2 && height >= 2) {
    ...
    maxDensity = 1;
    for (var _y3 = 0; _y3 < downScaledHeight; ++_y3) {
      for (var _x3 = 0; _x3 < downScaledWidth; ++_x3) {
        maxDensity = Math.max(maxDensity, downScaledInitialDensities[_y3 * downScaledWidth + _x3] = initialDensities[(2 * _y3 + 0) * width + (2 * _x3 + 0)] + initialDensities[(2 * _y3 + 0) * width + (2 * _x3 + 1)] + initialDensities[(2 * _y3 + 1) * width + (2 * _x3 + 0)] + initialDensities[(2 * _y3 + 1) * width + (2 * _x3 + 1)]);
      }
    }
    ...
    if (inflateToFit) {
      for (var _y4 = 0; _y4 < height; ++_y4) {
        for (var _x4 = 0; _x4 < width; ++_x4) {
          if (initialDensities[_y4 * width + _x4] !== 0.0) {
            ...
            expectedRuntime += _r3 * _r3 * Math.PI;
            newBounds_l = Math.min(newBounds_l, _x4 - _r3);
            newBounds_r = Math.max(newBounds_r, _x4 + _r3);
            newBounds_t = Math.min(newBounds_t, _y4 - _r3);
            newBounds_b = Math.max(newBounds_b, _y4 + _r3);
          }
        }
      }
    } else {
      for (var _y5 = 0; _y5 < height; ++_y5) {
        for (var _x5 = 0; _x5 < width; ++_x5) {
          if (initialDensities[_y5 * width + _x5] !== 0.0) {
            var r = Math.floor(Math.sqrt(Math.log(cutoffIntensity / initialDensities[_y5 * width + _x5]) / normalizedGaussScale)) - 1;
            r = Math.max(1, Math.min(maxExtend, r));

            var trimX = Math.min(r, Math.min(_x5, width - _x5)) / r;
            var trimY = Math.min(r, Math.min(_y5, height - _y5)) / r;
  ...
  for (var _y6 = 0; _y6 < maxExtend; ++_y6) {
    for (var _x6 = 0; _x6 < maxExtend; ++_x6) {
      gauss[_y6 * maxExtend + _x6] = Math.exp(normalizedGaussScale * (_x6 * _x6 + _y6 * _y6));
    }
  }
  ...
  if (inflateToFit) {
    for (var _y7 = 0; _y7 < height; ++_y7) {
      for (var _x7 = 0; _x7 < width; ++_x7) {
        if (initialDensities[_y7 * width + _x7] !== 0.0) {
          ...
          for (var _yy = _y7 - _yExtend, _yend = _y7 + _yExtend; _yy <= _yend; ++_yy) {
            // Compute horizontal extend of gaussian at height yy - y => xExtend
            var _xExtend = Math.floor(Math.sqrt(_sqYExtend - (_yy - _y7) * (_yy - _y7)));

            for (var _xx = _x7 - _xExtend, _xend = _x7 + _xExtend; _xx <= _xend; ++_xx) {
              densities[(_yy - newBounds_t) * densitMapWidth + _xx - newBounds_l] += _initialDensities_xy * gauss[Math.abs(_y7 - _yy) * maxExtend + Math.abs(_x7 - _xx)];
            }
          }
        }
      }
    ...
    for (var _y8 = 0; _y8 < height; ++_y8) {
      for (var _x8 = 0; _x8 < width; ++_x8) {
        if (initialDensities[_y8 * width + _x8] !== 0.0) {
          var initialDensities_xy = initialDensities[_y8 * width + _x8];
          ...
          if (yExtend <= 0) {
            densities[_y8 * densitMapWidth + _x8] += initialDensities_xy;
            continue;
          }
          // var yExtend = ext[initialDensities_xy - 1] - 1; // Get precomputed yExtend
          var sqYExtend = yExtend * yExtend;

          for (var yy = Math.max(0, _y8 - yExtend), yend = Math.min(densitMapHeight - 1, _y8 + yExtend); yy <= yend; ++yy) {
            // Compute horizontal extend of gaussian at height yy - y => xExtend
            var xExtend = Math.floor(Math.sqrt(sqYExtend - (yy - _y8) * (yy - _y8)));

            for (var xx = Math.max(0, _x8 - xExtend), xend = Math.min(densitMapWidth - 1, _x8 + xExtend); xx <= xend; ++xx) {
              densities[yy * densitMapWidth + xx] += initialDensities_xy * gauss[Math.abs(_y8 - yy) * maxExtend + Math.abs(_x8 - xx)];
            }
          }
        }
      }
  ...
  for (var _y9 = 0, _i = 0; _y9 < densitMapHeight; ++_y9) {
    for (var _x9 = 0; _x9 < densitMapWidth; ++_x9, ++_i) {
      var density = logScale ? Math.log(densities[_i]) : densities[_i];
      if (density > 0.0) {
        densities[_i] = density;
        minDensity = Math.min(minDensity, density);
        maxDensity = Math.max(maxDensity, density);
        newBounds_l = Math.min(newBounds_l, _x9);
        newBounds_r = Math.max(newBounds_r, _x9);
        newBounds_t = Math.min(newBounds_t, _y9);
        newBounds_b = Math.max(newBounds_b, _y9);
      } else {
        densities[_i] = 0.0;
      }
    }
...
function findRepresentativePointsND(dataset, densityMap, k, dist) {
  ...
  for (var _c = 0; _c < nc; ++_c) {
    scales[_c] = 1 / (dataset.columns[_c].maximum - dataset.columns[_c].minimum);
    offsets[_c] = -dataset.columns[_c].minimum * scales[_c];
  }
  ...
  for (var i = 0; i < n; ++i) {
    for (var _c2 = 0; _c2 < nc; ++_c2) {
      p[_c2] = data[i * nc + _c2] * scales[_c2] + offsets[_c2];
    }
...
function findClosePointOfLowDensity(dataset, d0, d1, p, densityMap, stencilMap, minDistX, minDistY) {
  ...
  for (var _y10 = imgymin; _y10 < imgymax; ++_y10) {
    for (var _x10 = imgxmin; _x10 < imgxmax; ++_x10) {
      stencil[(_y10 - ymin) * stencilStride + (_x10 - xmin)] = 1;
    }
  }
  ...
  var sqdx, sqdy;
  for (var _y11 = ymin; _y11 < ymax; ++_y11) {
    for (var _x11 = xmin; _x11 < xmax; ++_x11) {
      if (stencil[(_y11 - ymin) * stencilStride + (_x11 - xmin)] === 0) {
        sqdx = Math.pow(_x11 - p0, 2);
        sqdy = Math.pow(_y11 - p1, 2);
        if (sqdx > sqMinDistX && sqdy > sqMinDistY) {
          var sqDensity = _x11 >= 0 && _x11 < width && _y11 >= 0 && _y11 < height ? Math.pow(densityOffset + densities[_y11 * width + _x11] * densityScale, 2) : sqDensityOffset;
          var sqDist = sqdx + sqdy;
          var penalty = 1e10 * sqDensity + sqDist;
          if (penalty < closestPointPenalty) {
            closestPointPenalty = penalty;
            closestPoint = [_x11, _y11];
          }
        }
      }
    }
...
function findClosePointOfLowDensityND_descend(dataset, p, densityMap, minDist) {
  ...
  for (var _c3 = 0; _c3 < nc; ++_c3) {
    start[_c3] = (data[p * nc + _c3] - dataset.columns[_c3].minimum) * size / (dataset.columns[_c3].maximum - dataset.columns[_c3].minimum);
  }
...
function computeClusterMap_method1(dataset, d0, d1, densityMap) {
  ...
  for (var _i2 = 0; _i2 < n; ++_i2) {
    var _p = data[_i2 * nc + d0] * s0 + o0;
    var _p2 = data[_i2 * nc + d1] * s1 + o1;

    var idx = Math.min(Math.floor(_p2 * height), height - 1) * width + Math.min(Math.floor(_p * width), width - 1);
    pointDensities[_i2] = densities[idx];
  }
...
function computeClusterMap_method2(dataset, d0, d1, densityMap) {
  ...
  for (var _i3 = 0; _i3 < n; ++_i3) {
    ...
    var _x12 = Math.clamp(Math.floor(_p3 * width), 0, width - 1);
    var _y12 = Math.clamp(Math.floor(_p4 * height), 0, height - 1);
    if (clustermap[_y12 * width + _x12] === 0) {
      // If clustermap[y * width + x] doesn't contain a cluster
      clustermap[_y12 * width + _x12] = currentClusterId;
      var _d = densities[_y12 * width + _x12]; // EDIT: Not sure if we need 'nd < n'

      if (--_x12 !== -1) {
        var _nd8 = densities[_y12 * width + _x12];
        if (_nd8 !== 0 && _nd8 < _d && clustermap[_y12 * width + _x12] === 0) {
          neighborQueue.push({ c: clustermap[_y12 * width + _x12] = currentClusterId, x: _x12, y: _y12, d: densities[_y12 * width + _x12] });
        }
      }

      ++_x12;
      if (++_x12 !== width) {
        var _nd9 = densities[_y12 * width + _x12];
        if (_nd9 !== 0 && _nd9 < _d && clustermap[_y12 * width + _x12] === 0) {
          neighborQueue.push({ c: clustermap[_y12 * width + _x12] = currentClusterId, x: _x12, y: _y12, d: densities[_y12 * width + _x12] });
        }
      }
      ...
      --_x12;
      if (--_y12 !== -1) {
        var _nd10 = densities[_y12 * width + _x12];
        if (_nd10 !== 0 && _nd10 < _d && clustermap[_y12 * width + _x12] === 0) {
          neighborQueue.push({ c: clustermap[_y12 * width + _x12] = currentClusterId, x: _x12, y: _y12, d: densities[_y12 * width + _x12] });
        }
      }
      ...
      if (++_y12 !== height) {
        ...
        if (_nd11 !== 0 && _nd11 < _d && clustermap[_y12 * width + _x12] === 0) {
          neighborQueue.push({ c: clustermap[_y12 * width + _x12] = currentClusterId, x: _x12, y: _y12, d: densities[_y12 * width + _x12] });
        }
      }
  ...
  while (neighborQueue.length) {
    ...
    if (--x !== -1) {
      var _nd12 = densities[y * width + x];
      if (_nd12 !== 0 && _nd12 < d && clustermap[y * width + x] === 0) {
    ...
    ++x;
    if (++x !== width) {
      var _nd13 = densities[y * width + x];
      if (_nd13 !== 0 && _nd13 < d && clustermap[y * width + x] === 0) {
        neighborQueue.push({ c: clustermap[y * width + x] = id, x: x, y: y, d: densities[y * width + x] });
      }
    }
    ...
    --x;
    if (--y !== -1) {
      var _nd14 = densities[y * width + x];
      if (_nd14 !== 0 && _nd14 < d && clustermap[y * width + x] === 0) {
        neighborQueue.push({ c: clustermap[y * width + x] = id, x: x, y: y, d: densities[y * width + x] });
      }
    }
...
function computeClusterMap_method3(densityMap, d0, d1, options) {
  ...
  for (var _y13 = 0; _y13 < height; ++_y13) {
    ...
    for (var _x13 = 0; _x13 < width; ++_x13) {
      var _d2 = densities[_y13 * width + _x13];

      if (_d2 >= densityThreshold) {
        if (leftClusterId !== 0) {
          if (_y13 !== 0 && (topClusterId = clustermap[(_y13 - 1) * width + _x13]) !== 0 && topClusterId !== leftClusterId) {
            // Link clusters
          ...
          clustermap[_y13 * width + _x13] = leftClusterId;
        } else if (_y13 !== 0 && (topClusterId = clustermap[(_y13 - 1) * width + _x13]) !== 0) {
          clustermap[_y13 * width + _x13] = leftClusterId = topClusterId;
        } else {
          clusters.push(new ForwardList(clustermap[_y13 * width + _x13] = leftClusterId = clusters.length + 1));
        }
      } else {
  ...
  if (false) {
    ...
    for (var _y14 = 0; _y14 < height; ++_y14) {
      for (var _x14 = 0; _x14 < width; ++_x14) {
        if (clustermap[_y14 * width + _x14] !== 0 && (_x14 < width - 1 && clustermap[_y14 * width + _x14 - 1] === 0 || _x14 > 0 && clustermap[_y14 * width + _x14 + 1] === 0 || _y14 > 0 && clustermap[(_y14 - 1) * width + _x14] === 0 || _y14 < height - 1 && clustermap[(_y14 + 1) * width + _x14] === 0)) {
          neighborQueue.push({ c: clustermap[_y14 * width + _x14], x: _x14, y: _y14, d: densities[_y14 * width + _x14] });
        }
      }
    }
    ...
    while (neighborQueue.length) {
      ...
      if (--x !== -1) {
        var _nd15 = densities[y * width + x];
        if (_nd15 !== 0 && clustermap[y * width + x] === 0) {
          neighborQueue.push({ c: clustermap[y * width + x] = id, x: x, y: y, d: _nd15 = densities[y * width + x] });
      ...
      ++x;
      if (++x !== width) {
        var _nd16 = densities[y * width + x];
        if (_nd16 !== 0 && clustermap[y * width + x] === 0) {
          neighborQueue.push({ c: clustermap[y * width + x] = id, x: x, y: y, d: _nd16 = densities[y * width + x] });
      ...
      --x;
      if (--y !== -1) {
        var _nd17 = densities[y * width + x];
        if (_nd17 !== 0 && clustermap[y * width + x] === 0) {
          neighborQueue.push({ c: clustermap[y * width + x] = id, x: x, y: y, d: _nd17 = densities[y * width + x] });
