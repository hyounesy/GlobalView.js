...
var $ = require('jquery'); // alternative: a
window.jQuery = $;
require('jquery-csv');
...
function Dataset() {
  ...
  */
  ...
  this.requestDensityMap = function (d0, d1, size, options, ondone) {
    ...
    if (isAsync) {
      ...
      if (!densityMap) {
        ...
        var p = new Parallel([libUtility.makeCloneable(_histogram), new libAlgorithm.DensityMapOptions(options)], { evalPath: 'eval.js' });
  ...
  this['inflate'] = this.inflate = function (factor, densityMapChain) {
    ...
    for (var _i = 0, _len = n * nc; _i < _len; ++_i) {
    ...
    for (var _i2, i_inflated = n, _len2 = n * nc; i_inflated < n_inflated; ++i_inflated) {
      _i2 = i_inflated % n;
    ...
    if (this.names !== null) {
      ...
      for (var _i3 = 0, _len3 = n; _i3 < _len3; ++_i3) {
        names_inflated[_i3] = names[_i3];
      }
      for (var index = 0, _i_inflated = n, _len4 = n * nc; _i_inflated < n_inflated; ++_i_inflated) {
        names_inflated[_i_inflated] = 'generated datapoint ' + ++index;
      }
    ...
    if (this.imageFilenames !== null) {
      ...
      for (var _i4 = 0, _len5 = n; _i4 < _len5; ++_i4) {
        imageFilenames_inflated[_i4] = imageFilenames[_i4];
      }
      for (var _i_inflated2 = n, _len6 = n * nc; _i_inflated2 < n_inflated; ++_i_inflated2) {
  ...
  this['save'] = this.save = function (filename, nameColumn, nameColumnLabel) {
    ...
    for (var _c2 = 0, _ci = 0; _c2 < csv_nc; ++_c2, ++_ci) {
      if (_c2 === nameColumn) {
        header[_c2] = nameColumnLabel;
        --_ci;
      } else {
        header[_c2] = this.columns[_ci].label;
      }
    }
    csv[0] = header;
...
function RandomDataset(n, nc, onload) {
  ...
  for (var _i5 = 0; _i5 < nc; ++_i5) {
    this.columns.push({ minimum: 0, maximum: 1, label: generateColumnName(_i5, nc) });
    this.dataVectors.push(new DataVector(this, _i5));
  }
...
function CsvDataset(file, options, onload) {
  ...
  var parseCsv = function parseCsv(csv) {
    ...
    var i, di;
    for (var _c3 = 0, _ci2 = 0; _c3 < data[0].length; ++_c3, ++_ci2) {
      if (_c3 === options['nameColumn']) {
        --_ci2;
        continue;
      }
      ...
      for (i = firstRow, di = 0; i < data.length; ++i, ++di) {
        ...
        var _value = data[i][_c3];
        var _fvalue = parseData(_value);
        if (isNaN(_fvalue)) {
          isNumeric = false;
          break;
        }
        ...
        min = Math.min(min, _fvalue);
        max = Math.max(max, _fvalue);
      }
      ...
      if (!isNumeric) {
        ...
        for (i = firstRow, di = 0; i < data.length; ++i, ++di) {
          ...
          var value = data[i][_c3];
          var cls = _valueMap[value];
          var fvalue;
          if (typeof cls === 'undefined') {
            valueList.push(value);
            fvalue = _valueMap[value] = valueIdx++;
          } else {
            fvalue = cls;
          }
      ...
      dataset.columns.push({ minimum: min, maximum: max, label: columnLabels ? columnLabels[_ci2] : options['hasHeader'] ? data[0][_c3] : generateColumnName(_ci2, nc), values: valueList });
