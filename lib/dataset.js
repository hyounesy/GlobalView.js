...
function Dataset() {
  ...
  */
    ...
    if (isAsync) {
      ...
      if (!densityMap) {
        ...
        p.spawn(function (params) {
          ...
          var computedDensityMap = new libAlgorithm.DensityMap(pDensityMap);
          // Free histogram
          histogram = null;
  ...
  this.inflate = function fInflate(factor, densityMapChain) {
    ...
    for (var iInflated = n; iInflated < nInflated; iInflated += 1) {
      // i = iInflated % n;
    ...
    if (this.names !== null) {
      ...
      var namesInflated = new Array(nInflated);
      for (var _i2 = 0, _len2 = n; _i2 < _len2; _i2 += 1) {
        namesInflated[_i2] = names[_i2];
      }
      for (var index = 0, _iInflated = n; _iInflated < nInflated; _iInflated += 1) {
        namesInflated[_iInflated] = 'generated datapoint ' + (index += 1);
      }
    ...
    if (this.imageFilenames !== null) {
      ...
      var imageFilenamesInflated = new Array(nInflated);
      for (var _i3 = 0, _len3 = n; _i3 < _len3; _i3 += 1) {
        imageFilenamesInflated[_i3] = imageFilenames[_i3];
      }
      for (var _iInflated2 = n; _iInflated2 < nInflated; _iInflated2 += 1) {
        imageFilenamesInflated[_iInflated2] = imageFilenames[_iInflated2 % n];
      }
  ...
  this.save = function fSave(filename, nameColumnIndex, nameColumnLabel) {
    ...
    var header = new Array(csvNumCols);
    for (var c = 0, ci = 0; c < csvNumCols; c += 1, ci += 1) {
      if (c === nameColumn) {
    ...
    for (var i = 0; i < this.length; i += 1) {
      var row = new Array(csvNumCols);
      for (var _c3 = 0, _ci = 0; _c3 < csvNumCols; _c3 += 1, _ci += 1) {
        if (_c3 === nameColumn) {
...
function RandomDataset(n, nc, onload) {
  ...
  for (var _i4 = 0; _i4 < nnc; _i4 += 1) {
    this.fdata[_i4] = Math.random();
  }
    ...
    if (varOptions.autoDetect) {
      if (libUtility.isUndefined(varOptions.hasHeader)) {
        ...
        var firstRowOnlyStrings = data[0].every(function (value) {
          return Number.isNaN(parseData(value));
        });
