...
function DepthFirstSearch(problem) {
	...
	var node = startnode;
	if (problem.isGoalState(node.state)) return node.state; // Start state is goal
	...
	while (true) {
		...
		if (fringe.length == 0) return null; // Goal not found
...
function SimpleAStarSearch(problem, heuristic) {
	...
	while (true) {
		problem.forEachSuccessor(node.state, function (successor, successor_cost) {
			...
			if (CHECK_CONSISTENCY && g + h < node.f) throw "Inconsistency found in A*-search heuristic";
			var successor_hash = problem.computeHash(successor);
			if (libUtility.isUndefined(closed[successor_hash])) {
...
function SimpleGreedySearch(problem) {
	...
	if (problem.isGoalState(state)) return state; // Start state is goal
	...
	while (true) {
		...
		if ((state = cheapestSuccessor) === null) return null; // Goal not found
		if (problem.isGoalState(state)) return state; // Goal found
	}
}