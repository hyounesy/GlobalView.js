...
exports.validateColormap = validateColormap;
...
var COLORMAP_WIDTH = exports.COLORMAP_WIDTH = 10; // [pixel]
...
function Colormap(gl, globalView) {
  ...
  this.render = function (flipY, plotBounds) {
    ...
    tickLabelLeft = Math.ceil(plotBounds.x + plotBounds.width + COLORMAP_WIDTH + axis.tickLength + 10 + tickLabelLeft);

    // >>> Draw axis label

    if (axis.label) {
      gl.drawText(axis.label, tickLabelLeft, gl.height - plotBounds.y - plotBounds.height / 2, 'topcenter', -Math.PI / 2);
    }
  };

  function checkOverlap() {
    // Minimum distance between tick labels in pixel
    var MIN_TICK_LABEL_DISTANCE = gl.measureTextWidth('  ');
...
function validateColor(color) {
  if (libUtility.isString(color)) {
    ...
    var rgb = libUtility.hexToRgb(color);
    if (rgb !== null && rgb.r >= 0x00 && rgb.r <= 0xFF && rgb.g >= 0x00 && rgb.g <= 0xFF && rgb.b >= 0x00 && rgb.b <= 0xFF) {
      return true;
    } // color is hex color
...
function parseColor(color) {
  if (libUtility.isString(color)) {
    ...
    var rgb = libUtility.hexToRgb(hex || color);
...
function validateColormap(colormap) {
  ...
  if (libUtility.isArray(colormap)) {
    ...
    if (libUtility.isString(colormap[0])) {
      for (var i = 0; i < colormap.length; i += 1) {
        var err = validateColor(colormap[i]);
        if (err !== true) {
          return err;
        }
      }
    ...
    for (var _i = 0; _i < colormap.length; _i += 1) {
      if (!libUtility.isNumber(colormap[_i]) || colormap[_i] < 0x00 || colormap[_i] > 0xFF) {
...
function parseColormap(colormap) {
  ...
  if (libUtility.isArray(colormap)) {
    ...
    if (libUtility.isString(colormap[0])) {
      var array = [];
      for (var i = 0; i < colormap.length; i += 1) {
        var color = parseColor(colormap[i]);
        if (color) {
