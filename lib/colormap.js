...
function Colormap(gl, globalView) {
	...
	var colormaps = {
		exhue: LoadTexture(gl, "/exhue.png", function () {
			globalView.invalidate();
		}), //function() { setTimeout(function() { globalView.invalidate(); }, 1000); }),
		rainbow: LoadTexture(gl, "/rainbow.png", function () {
			globalView.invalidate();
		}), //function() { setTimeout(function() { globalView.invalidate(); }, 1000); }),
	...
	this.render = function (flipY, plotBounds) {
		...
		if (flipY === true) mat4.scale(mattrans, mattrans, [1.0, -1.0, 1.0]);
	...
	this.onOptionsChanged = function (options) {
		...
		if (options['pointColor'] !== pointColor) {
			...
			if (pointColor === null) texColormap = colormaps.exhue;else {
				...
				if (c) texColormap = LoadTextureFromByteArray(gl, c, c.length / 4, 1);
			}
		}
	};
...
function validateColor(color) {
	if (isString(color)) {
		if (!isUndefined(colorNameToHex(color))) return true; // color is known color name
		var rgb;
		if ((rgb = hexToRgb(color)) !== null && rgb.r >= 0x00 && rgb.r <= 0xFF && rgb.g >= 0x00 && rgb.g <= 0xFF && rgb.b >= 0x00 && rgb.b <= 0xFF) return true; // color is hex color
...
function parseColor(color) {
	...
	if (isArray(color)) return color.length >= 4 ? new Uint8Array([color[0], color[1], color[2], color[3]]) : null;
...
function validateColormap(colormap) {
	...
	if (isString(colormap)) return validateColor(colormap);
	...
	if (isArray(colormap)) {
		...
		if (isString(colormap[0])) {
			...
			for (var i = 0; i < colormap.length; ++i) {
				if (!isNumber(colormap[i]) || colormap[i] < 0x00 || colormap[i] > 0xFF) return "Colormap array must contain numbers between 0 and 255.";
			}return true;
		}
	}
...
function parseColormap(colormap) {
	if (isString(colormap)) return parseColor(colormap);
	...
	if (isArray(colormap)) {
		...
		} else if (isNumber(colormap[0])) return new Uint8Array(colormap);
	}
