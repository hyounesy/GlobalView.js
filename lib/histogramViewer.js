...
function HistogramViewer(gl, globalView) {
  ...
  function createHistogram(pAxis, pDataset, d, numBins) {
    ...
    var v3Set = function v3Set(pi, x, y) {
      var i = pi * 3;
      positions[i] = x;i += 1;
      positions[i] = y;i += 1;
      positions[i] = 0.0;i += 1;
    };
    for (var b = 0, i = -1, xScale = 1 / numBins; b < numBins; b += 1) {
    ...
    v3Set(0, 0, 0);
    for (var _b = 0, _i = 0, _xScale = 1 / numBins; _b < numBins;) {
      var _y = axis.histogram.data[_b] / axis.histogram.maximum;

      v3Set(_i += 1, _b * _xScale, _y);
      v3Set(_i += 1, (_b += 1) * _xScale, _y);
      v3Set(_i += 1, _b * _xScale, 0);
    }
