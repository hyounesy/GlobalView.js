...
exports.HistogramViewer = HistogramViewer;
...
function HistogramViewer(gl, globalView) {
  ...
  var axes = [{
    histogram: null,
    d: -1,
    meshHistogram: new libGraphics.Mesh(gl, new Float32Array(0), null, null, null, null, null, gl.TRIANGLES),
    meshLineHistogram: new libGraphics.Mesh(gl, new Float32Array(0), null, null, null, null, null, gl.LINE_STRIP)
  }, {
    histogram: null,
    d: -1,
    meshHistogram: new libGraphics.Mesh(gl, new Float32Array(0), null, null, null, null, null, gl.TRIANGLES),
  ...
  this.render = function (flipY, tf, plotBounds) {
    ...
    var scl = libGlMatrix.vec3.create();
    ...
    if (options.showYAxisHistogram && axes[1].histogram) {
      ...
      sdrLine.matWorldViewProj(mattrans);
      sdrLine.color.apply(sdrLine, [gl.foreColor[0], gl.foreColor[1], gl.foreColor[2], 0.5]);
      _axis.meshHistogram.bind(sdrLine, null);
      _axis.meshHistogram.draw();

      sdrLine.color.apply(sdrLine, _toConsumableArray(gl.foreColor));
      _axis.meshLineHistogram.bind(sdrLine, null);
      _axis.meshLineHistogram.draw();
    ...
    if (options.showColormapHistogram && axes[2].histogram) {
      ...
      sdrLine.matWorldViewProj(mattrans);
      sdrLine.color.apply(sdrLine, [gl.foreColor[0], gl.foreColor[1], gl.foreColor[2], 0.5]);
      _axis2.meshHistogram.bind(sdrLine, null);
      _axis2.meshHistogram.draw();

      sdrLine.color.apply(sdrLine, _toConsumableArray(gl.foreColor));
      _axis2.meshLineHistogram.bind(sdrLine, null);
      _axis2.meshLineHistogram.draw();
  ...
  function createHistogram(pAxis, dataset, d, numBins) {
    ...
    var v3_set = function v3_set(pi, x, y) {
      var i = pi * 3;
      positions[i] = x;i += 1;
      positions[i] = y;i += 1;
      positions[i] = 0.0;i += 1;
    };
    for (var b = 0, i = -1, x_scale = 1 / numBins; b < numBins; b += 1) {
    ...
    v3_set(0, 0, 0);
    for (var _b = 0, _i = 0, _x_scale = 1 / numBins; _b < numBins;) {
      var _y = axis.histogram.data[_b] / axis.histogram.maximum;

      v3_set(_i += 1, _b * _x_scale, _y);
      v3_set(_i += 1, (_b += 1) * _x_scale, _y);
      v3_set(_i += 1, _b * _x_scale, 0);
    }
