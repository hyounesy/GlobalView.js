   ...
   'normalize': FormulaCompiler.types.float,*/
		...
		function buildAST() {
			...
			function prefixOpAST(op) {
				if (!isString(curTok)) return error("Expected variable after prefix operator '" + op + "'");
      ...
      lhs = lhs.concat(ifExpr, elseExpr); lhs.push('?');*/
						...
						if (isUndefined(returnType)) return error("Undefined operator " + binOp + " on FormulaCompiler.types " + lhs.type.name + " and " + rhs.type.name);
			...
			function identifierAST() {
				...
				while (curTok === '.') {
					...
					if (isUndefined(type)) return error("Undefined variable: " + identifier);
					var parentType = type;
					var member = type.members[curVal];
					if (isUndefined(member)) return error(parentType.name + " does not contain a member: " + curVal);
					type = member.type;
    ...
    case '++': case '--':
    	...
    	identifier.push('post' + curTok);
			...
			function varDeclAST(type, typeName) {
				...
				if (isUndefined(type)) return error("Unsupported type: " + typeName);

				// Update scope
				var prev = scope[curVal];
				if (!isUndefined(prev)) return error("Redefinition of variable: " + curVal);
    ...
    return { code: decl, type: type };*/
			...
			function functionAST(funcName) {
				...
				var returnType = functionsReturnTypes[funcSignature];
				if (isUndefined(returnType)) return error("Undefined function " + funcSignature);
...
function verboseTest(formula, symbols, symbolTypes) {
	...
	if (isString(code)) console.log("err: " + code);else {
...
function verify(formula, result) {
	...
	if (isString(code)) console.log("Formula '{0}' failed with error '{1}'".format(formula, code));else {
