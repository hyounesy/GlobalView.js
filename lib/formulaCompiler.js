...
var FormulaCompiler = exports.FormulaCompiler = {
  compile: function compile(formula, symbolTypes) {
    ...
    function buildAST() {
      ...
      function varDeclAST(pType, typeName) {
        var type = FormulaCompiler.types[typeName];
        if (libUtility.isUndefined(type)) {
      ...
      function listAST(termTok) {
        ...
        while (true) {
          ...
          code = code.concat(_expr.code);
          typeList.push(_expr.type);
      ...
      switch (code[IP]) {
        ...
        */
        case 'min(float, float)':
          stack[SP - 2] = Math.min(stack[SP - 2], stack[SP -= 1]);break;
        case 'max(float, float)':
          stack[SP - 2] = Math.max(stack[SP - 2], stack[SP -= 1]);break;
        /*
        ...
        */
        ...
        case '@[]':
          ...
          break;
        case '.':
          scope = scope[stack[SP -= 1]]; // Dereference member
...
function verboseTest(formula, symbols, symbolTypes) {
  var code = FormulaCompiler.compile(formula, symbolTypes || {});

  libUtility.consoleLog('formula: ' + formula);
  if (libUtility.isString(code)) {
    libUtility.consoleLog('err: ' + code);
  } else {
    var globalScope = symbols || {};
    libUtility.consoleLog('code: ' + code.map(function (c) {
