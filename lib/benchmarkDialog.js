...
function BenchmarkDialog() {
  ...
  function startBenchmark() {
    ...
    for (var i in allElements) {
      if (allElements[i].className == 'option') {
        if (allElements[i].onchange) allElements[i].onchange(allElements[i]);else if (allElements[i].oninput) allElements[i].oninput(allElements[i]);
      }
    }numBenchmarks = 1;
    benchmarkCounter = 0;
    for (option in benchmarkOptions) {
      numBenchmarks *= benchmarkOptions[option].length;
    }if (SAVE_SCREENSHOTS) zip = new JSZip();
    ...
    if (reportProgress(0, 0)) setTimeout(startBenchmarkPass, 0);else cancelBenchmark();
  }
  ...
  function startBenchmarkPass() {
    ...
    if (currentOptions.N !== n) globalView.load(new RandomDataset(n = currentOptions.N, 2), 0, 1, 1, 1);
    ...
    if (reportProgress(0, 0)) setTimeout(renderBenchmark, 0);else cancelBenchmark();
  }
  ...
  function renderBenchmark() {
    ...
    if (reportProgress(benchmarkCounter / numBenchmarks, passTime / SECONDS_PER_BENCHMARK)) setTimeout(passTime < SECONDS_PER_BENCHMARK ? renderBenchmark : finishBenchmarkPass, 0);else cancelBenchmark();
  }
  ...
  function finishBenchmarkPass() {
    ...
    var getKeyByIndex = function getKeyByIndex(map, idx) {
      for (key in map) {
        if (idx-- === 0) return key;
      }return null;
    };
    ...
    if (reportProgress(++benchmarkCounter / numBenchmarks, 1) && option !== null) setTimeout(startBenchmarkPass, 0);else if (option !== null) cancelBenchmark();else finishBenchmark();
  }
  ...
  function finishBenchmark() {
    ...
    } else libUtility.download("benchmark.csv", "data:text/csv;charset=utf-8," + encodeURIComponent($.csv.fromArrays(csv)));
  }
}