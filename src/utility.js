...
/**
 * Enables or disables console outputs generated by the GlobalView.
 * @param {boolean} enable whether to enable or disable console outputs
 */
export function enableConsoleOutputs(enable) {
  enableConsole = enable;
}

/**
 * Enables or disables alert messages generated by the GlobalView.
 * @param {boolean} enable whether to enable or disable alert messages
 */
export function enableAlerts(enable) {
  enableAlert = enable;
}
...
/**
 * Outputs a console warn message
...
/**
 * Checks if a variable is an instanced object
 * @param {*} x variable to check
 * @returns {boolean} true if variable is an instanced object
 */
 ...
 * @param {string} name
 * @param {*} value
 * @param {number} days expiry date in days
 */
...
export function getParameterByName(name, url) {
  ...
  const varName = name.replace(/[\[\]]/g, '\\$&'); // eslint-disable-line no-useless-escape
  const regex = new RegExp(`[?&]${varName}(=([^&#]*)|&|#|$)`);
  const results = regex.exec(varUrl);
  if (!results) {
    return null;
  }
  if (!results[2]) {
